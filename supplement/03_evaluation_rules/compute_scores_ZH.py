#!/usr/bin/env python3
"""
compute_scores.py（唯一权威评分实现）
====================================

【权威性声明】
--------------
本文件定义了 Prompt Drift Lab 中 **唯一且权威（canonical）**
的评分与聚合逻辑，用于将 judged_scores 目录中的评审结果
汇总为最终的 summary 表。

【接口约定（Contract）】
-----------------------
- 输入（Input）：
  一个包含 judged score 文件（JSON / JSONL）的目录路径
- 输出（Output）：
  一个 summary CSV 文件路径

【工程约束】
-----------
- 本文件被视为评测协议（Evaluation Protocol）的一部分，
  而非普通工具脚本。
- 其他脚本（例如 tools/reproduce_summary.sh）
  **只能调用本文件**，不得在其他位置重新实现评分或聚合逻辑。

【变更规则（Protocol Change Policy）】
---------------------------------------
- 对本文件的任何修改，均视为评测协议的变更（protocol change）。
- 所有变更必须：
  1) 明确进行版本标注
  2) 在 README / Supplement 中记录变更原因
  3) 重新运行完整评测流程以保证可复现性

【重要说明】
-----------
- 本文件下方的评分与聚合逻辑为核心研究构件，
  **不得删除、裁剪或重写**。
- 本声明仅用于明确权威性与工程契约，
  不改变任何现有评分语义或实验结果。
"""
