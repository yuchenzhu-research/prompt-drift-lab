...
Purpose:
- Recompute and aggregate evaluation results from JSON files under
  `04_results/cross_model_evaluation_results/valid_results`, including:

  - main_method (cross-model judging):
    judge_{judge}_bundle_{generator}_v2.json

  - supporting_method (self-judging):
    self_judge_{model}.json (or any .json with the same schema)

- Recomputed items include:
  - total = sum of five scoring dimensions
  - mean scores for explicit / implicit triggers
  - mean scores by prompt version and by question
  - inter-judge agreement

- This script does NOT re-score samples.
  It only validates, aggregates, and exports CSV files.

Compatible repository layout:
- 03_evaluation_rules/compute_scores_v2.py
- 04_results/cross_model_evaluation_results/valid_results/main_method/*.json
- 04_results/cross_model_evaluation_results/valid_results/supporting_method/*.json
"""
